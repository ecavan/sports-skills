...
def _normalize_standings_entries(standings_data):
    ...
    for entry in standings_data.get("entries", []):
        ...
        entries.append({
            ...
            "overtime_losses": stats.get("OTLosses", stats.get("otLosses", "")),
            "points": stats.get("points", ""),
            "games_played": stats.get("gamesPlayed", ""),
            "regulation_wins": stats.get("regulationWins", stats.get("regWins", "")),
...
def _normalize_standings(espn_data):
    ...
    NHL uses Conference -> Division -> Entries structure.
    """
    groups = []
    for child in espn_data.get("children", []):
        conference_name = child.get("name", child.get("abbreviation", ""))

        # Check for division sub-groups first (NHL standard)
...
def _normalize_roster(espn_data):
    ...
    NHL rosters may be grouped by position (like NFL) or flat (like NBA).
...
def _parse_athlete(athlete, default_position=""):
    ...
    return {
        ...
        "birthplace": athlete.get("birthPlace", {}).get("country", "") if isinstance(athlete.get("birthPlace"), dict) else "",
...
def get_leaders(request_data):
    ...
    if season:
        url = f"https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/{season}/types/2/leaders"
    else:
        url = "https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/leaders"

    cache_key = f"nhl_leaders:{season or 'current'}"
